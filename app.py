...
def fetch_all_data(target_date):
    ...
    days_ago = (datetime.today().date() - target_date).days
    
    start_swing = end_date - timedelta(days=365)
    start_gann = end_date - timedelta(days=3650)
    
    idx_daily = yf.download(list(indices.keys()), start=start_swing.strftime('%Y-%m-%d'), end=end_date.strftime('%Y-%m-%d'), progress=False)
    ...
    # Fetch Fundamental Data securely
    fundamentals = {}
    for tk in stocks.keys():
        try:
            info = yf.Ticker(tk).info
            pe = info.get('trailingPE', 0)
            if pe is None: pe = 0
            fundamentals[tk] = {'pe': pe}
        except:
            fundamentals[tk] = {'pe': 0}
            
    idx_intraday = pd.DataFrame()
    stock_intraday = pd.DataFrame()
    
    if days_ago < 59:
        start_intraday_days = days_ago + 5
        if start_intraday_days > 59: start_intraday_days = 59
        start_intraday = datetime.today().date() - timedelta(days=start_intraday_days)
    ...
    return idx_daily, idx_intraday, stock_intraday, stock_swing, stock_gann, fundamentals

with st.spinner(f"Synchronizing with NSE Servers & Ephemeris DB for {selected_date}..."):
    idx_daily, idx_intraday, stock_intraday, stock_swing, stock_gann, fund_data = fetch_all_data(selected_date)

target_dt = pd.to_datetime(selected_date)
if not idx_intraday.empty:
    try:
        if idx_intraday.index.tz is not None: idx_intraday.index = idx_intraday.index.tz_convert(None)
        if stock_intraday.index.tz is not None: stock_intraday.index = stock_intraday.index.tz_convert(None)
        end_of_target_day = target_dt + pd.Timedelta(days=1) - pd.Timedelta(seconds=1)
        idx_intraday = idx_intraday[idx_intraday.index <= end_of_target_day]
        stock_intraday = stock_intraday[stock_intraday.index <= end_of_target_day]
    except: pass

days_in_past = (datetime.today().date() - selected_date).days
if days_in_past >= 59:
    st.error(f"⚠️ Yahoo Finance limits Intraday (15-Minute) data to the last 60 days. You selected a date {days_in_past} days in the past, so the Intraday scanners cannot run. However, the Swing and Gann Cycle tables at the bottom will still work perfectly!")

# --- 0. LIVE HEADER PRICES ---
...
for ticker, name in indices.items():
    try:
        ...
        if temp_df.index.tz is not None: temp_df.index = temp_df.index.tz_convert(None)
        temp_df = temp_df[temp_df.index.date <= target_dt.date()]
        if len(temp_df) >= 2:
...
st.markdown("""<div class="explainer-box"><b>📖 How to use this:</b> Check this table during market hours. <br><b>🟢 BUY CE (Call):</b> Market is in an Uptrend but briefly pulled back (15M RSI < 40). <br><b>🔴 BUY PE (Put):</b> Market is in a Downtrend but briefly pumped (15M RSI > 60).</div>""", unsafe_allow_html=True)
...
st.markdown("""<div class="explainer-box"><b>📖 High Frequency Intraday Strategy:</b> Same as Index options, but for individual Stocks. Hold time is approx 90 minutes. <br><b>🔥 BUY CE:</b> Astro score is highly bullish, and 15M RSI pulled back to < 40. Target is 0.5% underlying move.<br><b>🩸 BUY PE:</b> Astro score is hostile, and the stock pumped to > 60 RSI.</div>""", unsafe_allow_html=True)
...
st.markdown("""<div class="explainer-box"><b>📖 How to use this:</b> Check this at <b>3:15 PM</b> to buy Equity delivery for multi-day swings.<br><b>🟢 STRONG BUY (Sniper):</b> Our 80% Win Rate setup. Strict Astro-Momentum breakout.<br><b>🟡 BUY (50-SMA Bounce):</b> Our 58% Win Rate setup. Stock pulled back to the 50-Day Moving Average support.</div>""", unsafe_allow_html=True)
...
# ---------------------------------------------------------
# 4. GANN ASTRO TIME CYCLES + FUNDAMENTAL FILTER
# ---------------------------------------------------------
st.markdown('<div class="section-title">🪐 GANN ASTRO CYCLES & FUNDAMENTALS</div>', unsafe_allow_html=True)
st.markdown("""
<div class="explainer-box" style="border-left: 4px solid #f59e0b;">
    <b>📖 The Ultimate Filter:</b> Scans 10 years of planetary cycle history, but ONLY approves the trade if the stock is fundamentally and technically sound!<br>
    <b>⚠️ REJECTED:</b> The Astro cycle flashed buy, but the stock is in a massive downtrend or has an absurdly high P/E ratio.<br>
    <b>✅ APPROVED:</b> Perfect historical Gann Cycle <b>AND</b> the fundamentals are pristine. Target is Dynamic based on historical average.
</div>
""", unsafe_allow_html=True)

if not stock_gann.empty:
    gann_html = """<div class="custom-table-container border-top-gann"><table><thead><tr>
        <th>TICKER</th><th>TODAY'S ANGLE</th><th>CYCLE WIN RATE</th><th>TREND & FUNDAMENTALS</th><th>DYNAMIC TARGET</th><th>DYNAMIC SL</th><th>ACTION</th>
        </tr></thead><tbody>"""
        ...
        for ticker in stocks.keys():
            try:
                ...
                if historical_returns:
                    ...
                    sig, act_class, dyn_tp, dyn_sl = "WAIT FOR CYCLE", "b-dark", "-", "-"
                    fund_text = "<span style='color:#94a3b8;'>Not matching cycle</span>"
                    
                    if win_rate >= 60 and avg_ret > 1.0:
                        close_now = float(df['Close'].iloc[-1])
                        sma_200 = float(df['Close'].rolling(200).mean().iloc[-1])
                        
                        pe_ratio = fund_data[ticker].get('pe', 0)
                        
                        trend_fail = close_now < sma_200
                        fund_fail = pe_ratio > 45
                        
                        if trend_fail or fund_fail:
                            sig, act_class = "⚠️ REJECTED", "b-red"
                            issues = []
                            if trend_fail: issues.append("Downtrend")
                            if fund_fail: issues.append(f"High P/E ({pe_ratio:.1f})")
                            fund_text = f"<span style='color:#ef4444; font-size:0.8rem;'>Failed: {', '.join(issues)}</span>"
                        else:
                            sig, act_class = "✅ APPROVED", "b-green"
                            fund_text = f"<span style='color:#10b981; font-size:0.8rem;'>Uptrend + Healthy P/E</span>"
                            dyn_tp = f"₹{close_now * (1 + (avg_ret/100)):,.1f} (+{avg_ret:.1f}%)"
                            dyn_sl = f"₹{close_now * (1 - ((avg_ret/2)/100)):,.1f} (-{avg_ret/2:.1f}%)"
                    ...
                    tp_color = "color:#10b981; font-weight:bold;" if sig == "✅ APPROVED" else "color:#475569;"
                    sl_color = "color:#ef4444; font-weight:bold;" if sig == "✅ APPROVED" else "color:#475569;"
                    
                    gann_html += f"<tr><td style='font-weight:bold;'>{ticker.replace('.NS','')}</td><td style='color:#f8fafc;'>{current_angle:.1f}°</td><td style='{wr_color}'>{win_rate:.1f}% Win</td><td>{fund_text}</td><td style='{tp_color}'>{dyn_tp}</td><td style='{sl_color}'>{dyn_sl}</td><td><span class='badge {act_class}'>{sig}</span></td></tr>"
            except: pass
    except:
